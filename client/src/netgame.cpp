...
void UpdateNetwork()
{
	...
	while(pkt = pRakClient->Receive())
	{
		...
		switch(packetIdentifier)
		{
			case ID_DISCONNECTION_NOTIFICATION:
				Log("Подключение было закрыто сервером.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;
			case ID_CONNECTION_BANNED:
				Log("Я забанен.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;			
			case ID_CONNECTION_ATTEMPT_FAILED:
				Log("Попытка подключения не удалась.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;
			case ID_NO_FREE_INCOMING_CONNECTIONS:
				Log("Сервер переполнен игроками.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;
			case ID_INVALID_PASSWORD:
				Log("Неверный пароль.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;
			case ID_CONNECTION_LOST:
				Log("Связь была потеряна.", iReconnectTime / 1);
				resetPools(1, iReconnectTime);
				break;
	...
	UpdatePlayerScoresAndPings(2, 3000);
}
